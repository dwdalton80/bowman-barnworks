...
// Prefix public asset paths with the Vite base URL so they resolve whether the
// site is served from a domain root or from a project subpath
// (e.g. https://dwdalton80.github.io/bowman-barnworks/).
  ...
  return (
    <main className="min-h-screen bg-[#f5ead2] text-[#24140f] selection:bg-[#b85a20] selection:text-[#1c100b]">
      <header className="absolute inset-x-0 top-0 z-40 text-[#fff3d8]">
        <div className="container flex items-center justify-between border-b border-white/20 py-5">
          <button onClick={() => scrollTo("top")} className="flex items-center gap-3 text-left" aria-label="Bowman Barnworks home">
            <img src={asset("/images/bowman-logo.png")} alt="Bowman Barnworks logo" className="h-32 w-32 rounded-full object-cover sm:h-40 sm:w-40" />
      ...
      <section id="top" className="relative flex min-h-[730px] items-end overflow-hidden bg-[#24140f] pb-20 pt-36 lg:min-h-[820px] lg:pb-28">
        <img src={asset("/images/bowman-sunset-windmill-hero.png")} alt="Bowman Barnworks windmill, gazebo, and homestead at sunset" className="absolute inset-0 h-full w-full object-cover object-[62%_center] opacity-90" />
        ...
        <div className="container relative z-10 grid items-end gap-10 lg:grid-cols-[auto_1fr] lg:gap-14">
      ...
      <section className="relative overflow-hidden py-24 text-[#fff3d8] lg:py-32"><img src={asset("/images/windmill-services.jpg")} alt="Bowman Barnworks windmill at dusk" className="absolute inset-0 h-full w-full object-cover" loading="lazy" /><div className="absolute inset-0 bg-gradient-to-r from-[#1d0e08]/95 via-[#2f1208]/80 to-[#7f3514]/55" /><div className="container relative grid gap-12 xl:grid-cols-2 xl:items-start xl:gap-14"><div className="xl:pr-8"><p className="eyebrow text-[#e3a14c]">What we do</p><h2 className="section-title mt-5 max-w-2xl text-[#fff3d8]">Craftsmanship<br /><em className="text-[#f2be6b]">with a job to do.</em></h2><p className="brand-condensed mt-8 text-sm uppercase tracking-[.28em] text-[#e6c8a7]">Rustic structures · lasting traditions</p></div><div className="grid gap-4 sm:grid-cols-3">{services.map((service) => <div key={service.number} className="group border border-white/20 bg-black/20 p-6 backdrop-blur-sm transition hover:-translate-y-1 hover:border-[#e3a14c]/80 hover:bg-black/35"><div className="font-display text-3xl text-[#e3a14c]">{service.number}</div><h3 className="mt-8 font-display text-xl leading-snug text-[#fff3d8]">{service.title}</h3><p className="mt-4 text-sm leading-6 text-[#e6d2bc]">{service.text}</p><ArrowUpRight className="mt-6 h-4 w-4 text-[#e3a14c] transition group-hover:translate-x-1 group-hover:-translate-y-1" /></div>)}</div></div></section>
